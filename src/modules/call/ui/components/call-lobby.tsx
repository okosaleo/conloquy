...
const DisabledVideoPreview = () => {
    ...
    return (
        ...
        participant={
            {
                name: data?.user.name ?? "",
                image: data?.user.image ?? generateAvatarUrl({seed: data?.user.name ?? "", variant: "initials"})
...
export default function CallLobby({ onJoin }: Props) {
  ...
  return (
    <div className="flex flex-col w-full items-center justify-center h-screen bg-radial from-gray-400 to-primary/80">
